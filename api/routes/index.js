...
module.exports = function (io) {
  io.on('connection', function (socket) {
    ...
    //socket for template 2
    socket.on('update.file.template2', data => {
      //console.log('[SOCKET] fileName : ' + data.fileName);
      //console.log('[SOCKET] data : ' + data.previewMarkdown);
      ...
      var obj = convertFileToJSON_template_2(data.previewMarkdown);

      //send for all clients except sender
      socket.broadcast.emit("file.changed.template2", { fileName: data.fileName, data: obj });
    })
...
function convertFileToJSON(data) {
  ...
  emergencyContacts = emergencyContacts.substring(0, emergencyContacts.indexOf('### AUVs') - 1);

  var vehiclesObj = [];
  var vehicles = data.substring(data.indexOf('### AUVs') + 9);
...
function convertFileToJSON_template_2(data) {
  var objectives = data.substring(data.indexOf('### Objectives') + 15);
  objectives = objectives.substring(0, objectives.indexOf('### Team') - 1);
  ...
  weather = weather.substring(0, weather.indexOf('### Tides') - 3);

  var tides = data.substring(data.indexOf('### Tides') + 14);
  tides = tides.substring(0, tides.indexOf('### AUVs') - 3);
